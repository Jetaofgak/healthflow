...
@app.post("/features/extract/{pseudo_id}")
def extract_patient_features(pseudo_id: str, db: Session = Depends(database.get_db)):
    # 1. Récupérer le patient anonymisé
    patient = db.query(models.DeidPatient).filter(models.DeidPatient.pseudo_id == pseudo_id).first()
    
    if not patient:
        raise HTTPException(status_code=404, detail="Patient anonymisé introuvable")

    # 2. Vérifier si features déjà calculées
    existing = db.query(models.PatientFeatures).filter(models.PatientFeatures.pseudo_id == pseudo_id).first()
    if existing:
        return {"message": "Features déjà extraites", "features": json.loads(existing.feature_vector_json)}

    # 3. Calculer les features
    feats = feature_engine.extract_features(patient.anonymized_data)

    # 4. Sauvegarder
    new_features = models.PatientFeatures(
        pseudo_id=pseudo_id,
        age=feats['age'],
